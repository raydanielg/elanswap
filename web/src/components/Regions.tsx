    ...
    <section id="regions" className="py-16 lg:py-24 bg-gray-50 border-y">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-end justify-between gap-4">
          <div>
            <h2 className="text-2xl sm:text-3xl font-bold tracking-tight text-gray-900">Regions</h2>
            <p className="mt-2 text-gray-600">Explore opportunities by region.</p>
          </div>
          <a href="/regions" className="text-sm text-gray-600 hover:text-[var(--color-secondary)]">View all</a>
        </div>
          ...
          <div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {items.map((r) => (
              <a
                key={r.slug}
                href={`/regions#${r.slug}`}
                aria-label={`Browse region ${r.name}`}
