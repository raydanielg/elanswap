...
export default function Categories() {
  ...
  const load = () => {
    setLoading(true);
    setError(null);
    fetch("https://swap.elanbrands.net/api/categories")
      .then(async (r) => {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {loading ? (
          ...
          <div className="mt-8 rounded-lg border bg-white p-5 text-sm text-red-600">
            Imeshindikana kupakia jamii. <button onClick={load} className="underline hover:text-red-700">Jaribu tena</button>
          </div>
        ) : items.length === 0 ? (
          <div className="mt-8 rounded-lg border bg-white p-5 text-sm text-gray-600">Hakuna jamii zinazopatikana.</div>
        ) : (
          <div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {items.map((c) => (
              <a
